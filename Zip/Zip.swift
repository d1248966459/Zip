...
public enum ZipError: Error {
    ...
    case goToFirstFileFail
    
    case openCurrendFileFail
    
    case getCurrentFileFail
    
    case incorrectFileName
    
    case fwriteError
    
    case closeCurrentFileFail
    
    case incorrectUnzipFileSize
    ...
    public var description: String {
        ...
        case .goToFirstFileFail: return "Failed to go to first file"
        case .openCurrendFileFail: return "Failed to open current file"
        case .getCurrentFileFail: return "Failed to get current file info"
        case .incorrectFileName: return "File name wrong"
        case .fwriteError: return "fwrite error"
        case .closeCurrentFileFail: return "Failed to close current file"
        case .incorrectUnzipFileSize: return "Incorrect unzip file size"
        case .openfileError(let errorcode): return "open file failed errnoCode: \(errorcode)"
...
public class Zip {
    ...
    // MARK: Unzip
    public class func unzipFile(_ zipFilePath: URL, destination: URL, overwrite: Bool, password: String?, progress: ((_ progress: Double) -> Void)? = nil, fileOutputHandler: ((_ unzippedFile: URL) -> Void)? = nil, unzipOutputHandler: ((_ filePath: String, _ unzippedFile: ZipError) -> Void)? = nil) throws {
        ...
        if unzGoToFirstFile(zip) != UNZ_OK {
            unzipOutputHandler?(path, ZipError.goToFirstFileFail)
            throw ZipError.goToFirstFileFail
        }
        repeat {
            ...
            if ret != UNZ_OK {
                unzipOutputHandler?("current position \(currentPosition)", ZipError.openCurrendFileFail)
                throw ZipError.openCurrendFileFail
            }
            ...
            if ret != UNZ_OK {
                ...
                unzipOutputHandler?("current position \(currentPosition)", ZipError.openCurrendFileFail)
                throw ZipError.getCurrentFileFail
            }
            ...
            if crc_ret == UNZ_CRCERROR {
                unzipOutputHandler?(fullPath, ZipError.closeCurrentFileFail)
                throw ZipError.closeCurrentFileFail
            }
            guard writeBytes == fileInfo.uncompressed_size else {
                unzipOutputHandler?(fullPath, ZipError.openfileError(errno))
                throw ZipError.incorrectUnzipFileSize
            }
